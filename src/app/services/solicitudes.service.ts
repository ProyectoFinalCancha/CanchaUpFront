...
private apiUrl = 'http://localhost:8080/restful/services/simple.SolicituService/actions/crearSolicitud/invoke';
  ...
  crearSolicitud(nuevaSolicitud:Solicitud): Observable<any> {
    ...
    const body = {
      diaString: { value: nuevaSolicitud.dia },
      telefono: { value: nuevaSolicitud.telefono },
      horarioSting: { value: nuevaSolicitud.horario },
    };
      ...
      .pipe(
        catchError((error) => {
          if (error instanceof HttpErrorResponse && error.status === 404) {
            console.error('La solicitud no pudo ser encontrada. Posiblemente ya ha sido eliminada.');
          } else {
